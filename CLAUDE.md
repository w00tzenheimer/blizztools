...
blizztools refresh-products                         # dry-run: report new/gone codes vs live Ribbit summary
blizztools refresh-products --write                 # regenerate ALL_PRODUCT_CODES (union of live + current)
blizztools refresh-products --write --prune         # also drop codes no longer live
```
...
`ALL_PRODUCT_CODES` is regenerated by `refresh-products` (in `main.py`), which fetches the live product list from `{BASE_URL}/v2/summary`, diffs it against the current catalog, and — with `--write` — rewrites the tuple between the `# === BEGIN/END AUTOGEN CODES ===` markers. It **unions** by default so hand-added legacy codes (e.g. `wow_alpha`, `wowdemo`) survive; `--prune` drops codes no longer live. Before writing it runs `find_enum_collisions` over the result and **refuses** if any two codes would produce the same enum name (the live listing contains case-only dupes such as `scor-beta-RC` vs `scor-beta-rc`); resolve by adding an override or pruning. The catalog is a point-in-time snapshot: the checked-in list was ~604 codes while the live summary had ~692, so run `refresh-products` rather than hand-adding when Blizzard publishes new products.
